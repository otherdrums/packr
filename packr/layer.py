"""
PackRLinear — Packed Residual linear layer.
...
class PackRLinear(nn.Module):
    """
    PackR linear layer with learnable LUT.
    ...
    def from_linear(cls, module: nn.Linear):
        """
        Convert a standard nn.Linear into a PackRLinear.
        ...
        packr.W_p.data = best_indices.contiguous()
        packr.lut.data = dequant_lut.contiguous().to(device=device, dtype=torch.float32)
        packr.W_f.data = (W_orig - quantized_orig).contiguous().to(device=device, dtype=torch.bfloat16)

        if module.bias is not None:
            packr.bias_f.data = module.bias.detach().to(device=device, dtype=torch.bfloat16)
