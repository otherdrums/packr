"""
PackR custom autograd function.
...
class PackRMatmulFunction(torch.autograd.Function):
    """
    Custom autograd for PackR matmul:  out = x @ (W_f + lut[W_p])
    ...
    - bias: handled outside (added in PackRLinear.forward, not here)
    ...
    def forward(ctx, x, W_p, W_f, lut, evict_cb=None, layer_name=None, offload_mgr=None):
        ...
        out = packr_matmul(x, W_p, W_f, lut, bias=None)
        return out
